...
A `choice` question takes `criteria` as an object mapping labels to descriptions, or a list of label strings.

A `score` question takes a list of level descriptions and returns the expected level index, starting at zero.

A `noul` question returns the probability that its statement is true.
